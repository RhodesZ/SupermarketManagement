        ...
        private void searchbutton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (searchcommannd() != "")
                {
                    ...
                    else if(mode.Text=="进价最高的商品")
                        data.Sort(new InPriceDown());
                    else if(mode.Text=="售价最高的商品")
                        data.Sort(new OutPriceDown());
                    else if(mode.Text=="利润最高的商品")
                        data.Sort(new ProfitDown());
                    else if(mode.Text=="利润率最高的商品")
                        data.Sort(new ProfitRateDown());
                    int index = this.dataGridView1.Rows.Add();
                    for (int i = 0; i < 16; i++)
                    {
                        this.dataGridView1.Rows[index].Cells[i].Value = data[0].data[i];
                    }
                    data.Clear();
                    sdr.Close();
                }
